...
public class Main implements ActionListener{
 ...
 /*By providing a private constructor you prevent class instances from being created in any place other than this very class.*/
  ...
  //sol =new Soldier();
    ...
    white=true;
    dest=0;
    check=new CheckMate(board,f);
    }
  private void setSource(int req ){
       this.req=req;
  }
   public int getSource(){
        return this.req;
 }
 private void setDestination(int r){
      this.dest=r;
 }
        ...
        public void mouseReleased(MouseEvent e)
               {
               	...
               	int req1 = y1*8+x1;
               	setDestination(req1);
               	 piece=new Piece(getSource(),getDestination(),getBoard());
               	 if(piece.isValid()) {
                 JButton K=(JButton)board.getComponent(req1);
                 JButton K1=(JButton)board.getComponent(getSource());
                 text1.setText("NONE");
                ...
                board.repaint();}}
                    ...
                    board.repaint();
               		 ...
               		 setTurn();
               		JOptionPane.showMessageDialog(f, 
                            "Please refer instruction for correct moves", 
                            "InValid Move", 
                            JOptionPane.WARNING_MESSAGE);  }
               	 if(check.checkForCheckMate()) {
               		 text1.setText("NONE");
       	             turn.setText("White");
                     f.dispose();
                     white=true;
